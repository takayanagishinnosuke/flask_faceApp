...
class BeOssan:
  ...
  def load_faces_from_image(self, image_path):
    ...
    if len(rects) == 0:
      ...
      print("おっさん顔の検出成功: {}".format(len(rects)))
  ...
  def transformation_from_points(self, t_points, o_points):
    ...
    # o_points: (origin points) 合成元の特徴点(合成画像)
...
## ----実行関数------
  def to_bean(self, image_path):
    ...
    cv2.imwrite('static/image/' + path + ext, base_image) ##--アウトプットのパス

    ##---以下は使わない--##
    # if self.before_after is True:
    #   before_after = numpy.concatenate((original, base_image), axis = 1)
    #   cv2.imwrite('before_after/' + path + ext, before_after) #--ビフォー・アフターのパス
  ...
  def _load_beans(self):
    # "合成元の画像をロードして、顔(特徴点など)を検出しておく"

    self.beans = []
    for image_path in glob.glob(os.path.join('static/beans', '*.jpg')): ##合成画像のpath
...
#   be_ossan = BeOssan()
#   be_ossan.to_bean('/Users/shinnosukeikeda/Documents/shinnosuke/Face_Flask_app/flask/flaskr/static/image/d51679-5-778304-0.jpeg') ## ココ
